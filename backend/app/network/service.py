...
class NetworkService:
    ...
    async def get_network(self, network_id: str, organization_id: str) -> Optional[RoadNetworkDTO]:
        ...
        if not model and (network_id in self._graph_cache):
            # In-memory network (e.g. offline import) — synthesize the record
            model = await self.repo.get_or_create_road_network(
                organization_id=organization_id,
                name=f"In-Memory Road Network {network_id[:8]}",
                source="OSM",
                version="v1.0",
            )
            model = model[0]
            model.id = uuid.UUID(network_id) if is_uuid(network_id) else model.id
        if not model:
            return None
    ...
    async def calculate_shortest_path(
        ...
        net = await self.repo.get_network_by_id(network_id, organization_id)
        if not net and network_id in self._graph_cache:
            # In-memory network (offline import): synthesize the record
            from uuid import UUID as _UUID
            try:
                net_uuid = _UUID(network_id)
            except (ValueError, TypeError):
                net_uuid = None
            from app.infrastructure.database.models.network import RoadNetwork as _RoadNetwork
            org_uuid = _UUID(organization_id) if is_uuid(organization_id) else None
            net = _RoadNetwork(
                id=net_uuid or uuid.uuid4(),
                organization_id=org_uuid or uuid.uuid4(),
                name="In-Memory Road Network",
                source="OSM",
                version="v1.0",
            )
        if not net:
