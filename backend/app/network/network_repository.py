...
class NetworkRepository:
    def __init__(self, db: Optional[AsyncSession] = None):
        self.db = db
        # No hard-coded fallback URL/key: when Supabase is not configured the
        # repository degrades to in-memory operation instead of network calls.
    ...
    async def get_or_create_road_network(
        ...
        # Fallback / Direct Supabase REST API. Wrap so a configured-but-
        # unreachable Supabase degrades to in-memory instead of raising.
        if not self.supabase_url:
            print("[NetworkRepository] Supabase not configured; returning in-memory network record.")
            net = RoadNetwork(id=uuid.uuid4(), organization_id=org_uuid, name=name, source=source, version=version)
            return net, True
        try:
            return await self._supabase_get_or_create(org_uuid, name, source, version)
        except Exception as e:
            print(f"[NetworkRepository] Supabase unreachable ({type(e).__name__}); using in-memory record.")
            net = RoadNetwork(id=uuid.uuid4(), organization_id=org_uuid, name=name, source=source, version=version)
            return net, True

    async def _supabase_get_or_create(self, org_uuid, name, source, version):
    ...
    async def save_nodes_and_edges(
        ...
        # Batch insert via Supabase REST API. A configured-but-unreachable
        # Supabase degrades gracefully: the caller still gets the counts.
        if not self.supabase_url:
            return len(nodes), len(edges)
        try:
            return await self._supabase_save_nodes_and_edges(network_id, nodes, edges)
        except Exception as e:
            print(f"[NetworkRepository] Supabase REST insert unreachable ({type(e).__name__}); skipping persistence.")
            return len(nodes), len(edges)

    async def _supabase_save_nodes_and_edges(self, network_id, nodes, edges):
