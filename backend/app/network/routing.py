...
def find_nearest_node(
    ...
    When require_connected is True, restricts candidates to the largest
    strongly connected component of G so the snapped node is guaranteed
    routable in both directions on the road graph.
    """
    if len(G.nodes) == 0:
        raise ValueError("Network graph contains no nodes.")

    # Determine candidate nodes
    if require_connected and G.is_directed() and len(G.nodes) > 1:
        if not hasattr(G, "_largest_scc"):
            sccs = list(nx.strongly_connected_components(G))
            G._largest_scc = max(sccs, key=len) if sccs else set(G.nodes)
        candidate_ids = G._largest_scc
    else:
...
def validate_route_feasibility(
    ...
    """
    Validate route feasibility against graph constraints:
    - Every consecutive node pair has a valid directed edge in G
    - No active blocked incident edges are traversed
    - No active road restrictions are violated
    - Route geometry is continuous and derived from real edges
    """
    blocked_set = set(str(e) for e in (blocked_edges or []))
    
    # Check node connectivity and edge existence
    for i in range(len(path_nodes) - 1):
        u, v = path_nodes[i], path_nodes[i + 1]
        if not G.has_edge(u, v):
            return {
                "valid": False,
                "used_graph_path": False,
                "used_fallback_geometry": False,
                "restrictions_valid": False,
                "blocked_edges_avoided": False,
                "error": f"No directed edge in graph between consecutive nodes {u} -> {v}",
                "geometry_source": "NONE",
            }
        e_data = G.get_edge_data(u, v)
        if 0 in e_data:
            e_data = e_data[0]
        e_id = str(e_data.get("id", f"edge_{u}_{v}"))
        if e_id in blocked_set or f"edge_{u}" in blocked_set:
            return {
                "valid": False,
                ...
                "restrictions_valid": False,
                "blocked_edges_avoided": False,
                "error": f"Route traverses blocked edge {e_id}",
                "geometry_source": "OSM_EDGE_GEOMETRY",
            }

    # Validate geometry coordinates
    validate_route_geometry(coordinates, path_nodes, edge_ids)
...
def compute_shortest_path(
    ...
    The edge cost uses static/dynamic travel time. Respects one-way edges and blocked edges.
    Returns complete route details and GeoJSON LineString geometry with [longitude, latitude] coordinates.
    Raises ValueError / nx.NetworkXNoPath if no legal road path exists.
    """
    ...
    # Build active view if blocked edges or weight filter needed
    active_graph = G
    if blocked_edge_ids:
        blocked_set = set(str(e) for e in blocked_edge_ids)
        # Create subgraph excluding blocked edges
        edges_to_exclude = []
        for u, v, d in G.edges(data=True):
            eid = str(d.get("id", f"edge_{u}_{v}"))
            if eid in blocked_set or f"edge_{u}" in blocked_set or d.get("blocked", False):
                edges_to_exclude.append((u, v))
        if edges_to_exclude:
            active_graph = G.copy()
            active_graph.remove_edges_from(edges_to_exclude)

    try:
        path_nodes = nx.dijkstra_path(active_graph, src_str, tgt_str, weight=weight_key)
        total_time_s = nx.dijkstra_path_length(active_graph, src_str, tgt_str, weight=weight_key)
    ...
    for i in range(len(path_nodes) - 1):
        ...
        if edge_data:
            ...
            if parsed_pts:
                ...
                # Graph edge endpoints only along traversable edge (u -> v)
    ...
    if len(line_coords_lng_lat) < 2:
        raise ValueError(f"Infeasible route: could not construct road geometry for path {src_str} -> {tgt_str}")

    # Validate route feasibility and constraints
    feasibility = validate_route_feasibility(
        G, path_nodes, edge_ids, line_coords_lng_lat, blocked_edges=blocked_edge_ids, restrictions=restrictions
    )
    if not feasibility["valid"]:
        raise ValueError(f"Route feasibility check failed: {feasibility.get('error', 'invalid route')}")
    ...
    return {
        ...
        "algorithm": "Dijkstra (Road-Constrained)",
        "geometry": {
            "type": "LineString",
