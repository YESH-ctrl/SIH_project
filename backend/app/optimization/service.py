...
class OptimizationService:
    ...
    def simulate_incident(self, request: Optional[IncidentSimulationRequestDTO] = None) -> IncidentSimulationResponseDTO:
        ...
        if target_route.edge_ids:
            edge_id = target_route.edge_ids[min(2, len(target_route.edge_ids) - 1)]
            # Find edge coordinates in graph
            for u, v, d in G.edges(data=True):
                if str(d.get("id")) == edge_id or str(d.get("osmid")) == edge_id:
                    road_name = d.get("name", road_name)
                    if "geometry" in d and d["geometry"].get("coordinates"):
                        coords = d["geometry"]["coordinates"]
                        lng, lat = coords[len(coords) // 2]
                    elif G.has_node(u):
                        lat, lng = G.nodes[u].get("y", lat), G.nodes[u].get("x", lng)
                    break
        elif target_route.stops and len(target_route.stops) >= 2:
        ...
        return IncidentSimulationResponseDTO(
            ...
            affected_vehicle_id=affected_veh,
            affected_route_id=f"route_{affected_veh}",
    ...
    def reroute_vehicle(self, request: RerouteRequestDTO) -> RerouteResponseDTO:
        ...
        blocked_edges = [request.affected_edge_id]

        reroute_evaluator = VRPEvaluator(
            G,
            request.network_id,
            depot,
            delivery_points,
