...
class RouteValidationMetadataDTO(BaseModel):
    valid: bool = True
    used_graph_path: bool = True
    used_fallback_geometry: bool = False
    restrictions_valid: bool = True
    blocked_edges_avoided: bool = True
    geometry_source: str = "OSM_EDGE_GEOMETRY"
    node_count: int = 0
    edge_count: int = 0
...
class VehicleRouteResultDTO(BaseModel):
    ...
    edge_ids: List[str] = []
    node_ids: List[str] = []
    validation: Optional[RouteValidationMetadataDTO] = None
    feasible: bool = True
