...
import { liveStore, type DataMode } from "@/services/liveStore";
import {
  Activity,
  Clock,
  Radio,
...
export function TopHeader({
  ...
  // Real backend data mode + realtime connection state (never hard-coded)
  const [dataMode, setDataMode] = useState<DataMode>(liveStore.snapshot.data_mode);
  const [realtimeConnected, setRealtimeConnected] = useState(liveStore.realtimeConnected);
  useEffect(() => {
    const unsubStatus = liveStore.subscribeStatus((s) => {
      setDataMode(s.dataMode);
      setRealtimeConnected(s.realtimeConnected);
    });
    liveStore.connectWebSocket();
    liveStore.refresh().catch(() => {});
    return unsubStatus;
  }, []);
  ...
  return (
    <header className="h-14 bg-[#080808] border-b border-neutral-800 px-4 flex items-center justify-between font-mono text-xs text-white selection:bg-[#c8ff00] selection:text-black select-none z-30 relative">
      ...
      <div className="flex items-center space-x-4">
        ...
        {/* Authoritative Data Mode Indicator (LIVE / SIMULATION / OFFLINE) */}
        <div
          className={`hidden md:flex items-center space-x-1.5 px-2.5 py-1 font-semibold border ${
            dataMode === "LIVE"
        ...
        >
          {dataMode === "SIMULATION" ? <FlaskConical size={12} /> : <Radio size={12} className={realtimeConnected && dataMode === "LIVE" ? "animate-pulse" : ""} />}
          <span>
            {dataMode === "LIVE" ? "LIVE DATA" : dataMode === "SIMULATION" ? "SIMULATION" : "OFFLINE"}
          </span>
        </div>
