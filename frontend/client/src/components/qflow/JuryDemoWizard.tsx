...
import { LayerVisibilityState, DEFAULT_LAYER_VISIBILITY } from "./mapLayerTypes";
...
function JuryDemoWizardInner({ onFinishDemo }: JuryDemoProps) {
  ...
  // Layer visibility for map — central single source of truth across steps
          ...
          {DEMO_STEPS.map((s) => {
            ...
            return (
              <div
                ...
                className={`p-2 border cursor-pointer text-left transition-all ${isDone && !isCurrent
                          ...
                          <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-slate-800">
                            {turnSteps.map((step, idx) => (
                              <div
                                ...
                                className={`p-2 border text-[11px] cursor-pointer transition-all flex items-start space-x-2.5 ${activeTurnIndex === idx
          ...
          <div className="pt-4 border-t border-slate-800 space-y-2 font-mono">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                variant="outline"
                className={`flex-1 text-[11px] font-bold rounded-none ${isAutoDemo
