...
function JuryDemoWizardInner({ onFinishDemo }: JuryDemoProps) {
  ...
  return (
    <div className="space-y-4 font-sans text-white">
      ...
      <div className="p-4 bg-gradient-to-r from-[#141a00]/70 via-[#0d0d0d] to-[#0d0d0d] border border-[#c8ff00]/35 font-mono text-xs space-y-3 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center space-x-2">
            <Sparkles size={16} className="text-[#c8ff00]" />
            <span className="font-extrabold text-[#c8ff00] text-sm tracking-tight uppercase">
              Q-FLOW JURY DEMONSTRATION WORKFLOW
            </span>
          </div>
          <div className="flex items-center space-x-2 text-[10px]">
            <span className="text-[#c8ff00] bg-[#c8ff00]/10 px-2 py-0.5 border border-[#c8ff00]/30 font-bold">
              SINGLE SOURCE OF TRUTH: SUPABASE / FASTAPI
            </span>
            <span className="text-slate-300 bg-slate-900 px-2 py-0.5 border border-slate-800 font-bold">
              {scenario?.network?.name || "Raipur Urban Network"} | {vehicleCount} VEHICLES | {deliveryCount} STOPS ({totalDem}/{totalCap}u)
            </span>
            {isAutoDemo && (
              <span className="text-[#c8ff00] bg-[#c8ff00]/10 px-2 py-0.5 border border-[#c8ff00]/40 font-bold animate-pulse">
                AUTO DEMO RUNNING
              </span>
            )}
          </div>
              ...
              <div
                ...
                className={`p-2 border cursor-pointer text-left transition-all ${
                  isDone && !isCurrent
                    ...
                    : isCurrent
                    ? "bg-[#c8ff00] text-black border-[#c8ff00] font-bold shadow-[0_0_12px_rgba(200,255,0,0.35)]"
                    : "bg-[#111111] border-slate-800 text-slate-400 hover:text-slate-200"
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-4 bg-[#0d0d0d] border border-slate-800 p-5 font-mono text-xs space-y-5 flex flex-col justify-between">
          <div>
            {/* STEP 1: TRANSPORTATION NETWORK */}
            {currentStep === 1 && (
              <div className="space-y-4">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 01 OF 08</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                  ...
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    ...
                    <span className="font-bold text-[#c8ff00]">{nodes.length.toLocaleString() || "9,053"} Nodes</span>
                  </div>
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    <span className="text-slate-400 block text-[9px]">ROAD EDGES</span>
                    <span className="font-bold text-[#c8ff00]">{edges.length.toLocaleString() || "11,806"} Edges</span>
                  </div>
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    <span className="text-slate-400 block text-[9px]">NETWORK ID</span>
                    <span className="font-bold text-[#c8ff00] text-[10px]">9cb256c8...</span>
                  </div>
                </div>

                <Button
                  onClick={nextStep}
                  className="w-full bg-[#c8ff00] hover:bg-[#b5e600] text-black font-bold uppercase text-xs shadow-lg py-3 flex items-center justify-center space-x-2"
                >
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 02 OF 08</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                  ...
                  <div className="p-2 bg-red-950/40 border border-[#ff4d2d]/40 text-red-300">
                    <span className="text-[9px] text-[#ff4d2d] block font-bold">🔴 DESTINATION (B)</span>
                    <span className="font-bold text-white">
                ...
                {abRouteError && (
                  <div className="p-3 bg-red-950/90 border border-[#ff4d2d]/60 text-red-300 text-xs font-mono space-y-1">
                    <div className="font-bold text-[#ff4d2d] flex items-center space-x-1.5">
                ...
                {abRoute && (
                  <div className="space-y-3">
                    <div className="p-3 bg-[#111111] border border-[#c8ff00]/40 text-slate-200 space-y-1.5 text-xs">
                      <div className="font-bold flex items-center justify-between">
                        <span className="text-emerald-400 flex items-center gap-1">
                          <CheckCircle2 size={13} /> ROUTE COMPUTED
                        </span>
                        <span className="text-[#c8ff00] font-bold">{abRoute.algorithm || "Dijkstra"}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-[11px] font-mono pt-1 text-slate-200">
                        <div>Distance: <span className="text-[#c8ff00] font-bold">{((abRoute.distance_meters ?? 0) / 1000).toFixed(2)} km</span></div>
                        <div>Travel Time: <span className="text-emerald-400 font-bold">{((abRoute.travel_time_seconds ?? 0) / 60).toFixed(1)} min</span></div>
                      ...
                      return (
                        <div className="space-y-2 border border-slate-800 bg-[#090b0e] p-3 font-mono">
                          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                            <span className="text-[#c8ff00] font-bold uppercase text-[11px] flex items-center gap-1.5">
                              <Compass size={14} className="text-[#c8ff00]" /> TURN NAVIGATION GUIDANCE
                            </span>
                            ...
                            {turnSteps.map((step, idx) => (
                              <div
                                ...
                                className={`p-2 border text-[11px] cursor-pointer transition-all flex items-start space-x-2.5 ${
                                  activeTurnIndex === idx
                                    ? "bg-[#141a00] border-[#c8ff00] text-white shadow-lg ring-1 ring-[#c8ff00]"
                              ...
                              >
                                ...
                                <div className="flex-1 space-y-0.5 min-w-0">
                                  ...
                                  <div className="text-[10px] text-slate-400 flex items-center space-x-2 font-mono pt-0.5">
                                    <span>Seg: <span className="text-[#c8ff00] font-bold">{step.distanceKm} km</span></span>
                                    <span>•</span>
            ...
            {currentStep === 3 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 03 OF 08</span>
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px]">
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    ...
                    <span className="font-bold text-[#c8ff00]">1 Central</span>
                  </div>
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    <span className="text-slate-400 block text-[9px]">FLEET</span>
                    <span className="font-bold text-[#c8ff00]">{vehicleCount} Vehicles</span>
                  </div>
                  <div className="p-2.5 bg-[#111111] border border-slate-800">
                    <span className="text-slate-400 block text-[9px]">DELIVERIES</span>
                    <span className="font-bold text-[#c8ff00]">{deliveryCount} Customer Stops</span>
                  </div>
                </div>

                {/* Depot Info */}
                <div className="p-3 bg-[#111111] border border-[#c8ff00]/30 text-slate-200 space-y-1 text-xs">
                  <div className="font-bold text-white uppercase text-[11px] flex items-center justify-between">
                    <span>🏢 CENTRAL RAIPUR DEPOT</span>
                    <span className="text-[10px] text-[#c8ff00] font-mono">QFLOW_DEPOT</span>
                  </div>
                  <div className="text-[10px] text-slate-300 pt-1">
                    Location: <span className="text-white font-bold">21.2517, 81.6294</span> | Capacity: <span className="text-[#c8ff00] font-bold">20 Vehicles</span>
                  </div>
                </div>

                {/* Capacity Utilization */}
                <div className="p-3 bg-[#111111] border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-[11px] font-bold">
                    <span className="text-slate-300 uppercase">FLEET LOAD UTILIZATION</span>
                    <span className="text-[#c8ff00] font-bold">{((totalDem / totalCap) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-slate-950 h-2 border border-slate-800 overflow-hidden">
                    <div className="bg-[#c8ff00] h-full" style={{ width: `${(totalDem / totalCap) * 100}%` }} />
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-[10px] text-slate-300 pt-0.5">
                    <div>Fleet Cap: <span className="text-[#c8ff00] font-bold">{totalCap} u</span></div>
                ...
                <Button
                  ...
                  className="w-full bg-[#c8ff00] hover:bg-[#b5e600] text-black font-bold uppercase text-xs shadow-lg py-3 flex items-center justify-center space-x-2"
                >
                  <span>CONTINUE TO BASELINE PLAN</span>
                  <ArrowRight size={14} />
                </Button>
              </div>
            )}
            ...
            {currentStep === 4 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 04 OF 08</span>
                ...
                <div className="p-3 bg-[#111111] border border-[#c8ff00]/30 text-slate-200 space-y-2 text-xs">
                  <div className="font-bold uppercase text-[11px] flex items-center justify-between text-[#c8ff00]">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div>Vehicles Used: <span className="text-[#c8ff00] font-bold">3 / 3</span></div>
                    <div>Deliveries Covered: <span className="text-emerald-400 font-bold">10 / 10</span></div>
                    <div>Fleet Capacity: <span className="text-[#c8ff00] font-bold">{totalCap} u</span></div>
                    <div>Total Demand: <span className="text-emerald-400 font-bold">{totalDem} u</span></div>
                  </div>
                </div>

                {/* Vehicle Stop Assignment Breakdown */}
                <div className="p-3 bg-[#111111]/80 border border-slate-800 space-y-1.5 text-xs">
                  ...
                  <div className="space-y-1 text-[11px] text-slate-300">
                    ...
                    <div>• <span className="text-[#c8ff00] font-bold">veh_03 (Swarm Gamma)</span>: Depot → dp_09, dp_10 → Depot (35u load)</div>
                  </div>
                </div>

                <Button
                  onClick={nextStep}
                  className="w-full bg-[#ff4d2d] hover:bg-[#e03e1f] text-white font-bold uppercase text-xs shadow-lg py-3 flex items-center justify-center space-x-2"
                >
            ...
            {currentStep === 5 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#ff4d2d] font-bold uppercase">STEP 05 OF 08</span>
                ...
                {incident ? (
                  <div className="space-y-3">
                    <div className="p-3 bg-red-950/50 border border-[#ff4d2d]/60 text-red-300 space-y-2 text-xs">
                      <div className="font-bold flex items-center space-x-1.5 text-[#ff4d2d]">
                      ...
                      <div className="space-y-1 text-[11px] font-mono text-slate-200">
                        ...
                        <div>Type: <span className="text-[#ff4d2d] font-bold">VEHICLE_ACCIDENT (HIGH)</span></div>
                        <div>Affected Vehicle: <span className="text-[#c8ff00] font-bold">veh_01 (Swarm Alpha)</span></div>
                        <div>Delay Impact: <span className="text-[#ff4d2d] font-bold">+78.0% Delay Increase</span></div>
                        <div>Status: <span className="text-[#ff4d2d] font-bold">AFFECTING ROUTE</span></div>
                      </div>
                    </div>
                    <Button
                      onClick={nextStep}
            ...
            {currentStep === 6 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 06 OF 08</span>
                ...
                <div className="p-3 bg-[#080808] border border-slate-800 text-[10px] text-slate-400 space-y-1.5 font-mono">
                  <div className="text-[#c8ff00] font-bold flex items-center justify-between">
                  ...
                  <div className="text-[#c8ff00] font-bold text-xs">F = α · T + β · D + γ · C + δ · P</div>
                ...
                {isOptimizing ? (
                  <div className="p-4 bg-[#141a00]/60 border border-[#c8ff00]/50 text-[#c8ff00] space-y-2">
                    <div className="flex items-center space-x-2 font-bold text-xs">
                      <Cpu size={16} className="animate-spin text-[#c8ff00]" />
                  ...
                  <div className="space-y-3">
                    <div className="p-3 bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 space-y-2 text-xs">
                      <div className="font-bold flex items-center justify-between">
                        ...
                        <span className="text-[#c8ff00] font-mono">{qpsoResult.runtime_ms} ms</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-[11px]">
            ...
            {currentStep === 7 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 07 OF 08</span>
                  ...
                  <div className="space-y-3">
                    <div className="p-4 bg-gradient-to-r from-emerald-950/60 to-slate-900 border border-emerald-500/50 space-y-3 font-mono text-xs">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px]">
                        ...
                        <div>Response Time: <span className="text-[#c8ff00] font-bold">{rerouteResult.reroute_runtime_ms || 42} ms</span></div>
                    ...
                    <div className="p-3 bg-emerald-950/40 border border-emerald-500/50 text-emerald-300 space-y-2 text-xs font-mono">
                      ...
                      <div className="space-y-1 pt-1 border-t border-emerald-900/60 text-[11px]">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-[#c8ff00] font-bold">• veh_03 (Swarm Gamma)</span>
                          <span className="text-emerald-400 font-bold bg-emerald-500/20 px-1.5 py-0.5 border border-emerald-500/40">✅ REROUTED</span>
                        </div>
                      </div>
                    </div>

                    <div className="p-3 bg-[#111111] border border-slate-800 text-[11px] text-slate-300 space-y-1 font-sans">
                      <div className="font-bold text-[#c8ff00] uppercase text-xs">Q-FLOW DYNAMIC RESPONSE COMPLETE</div>
                      <div>"Q-FLOW does not stop when the route is optimized. It adapts when the transportation network changes."</div>
                      <div className="text-[10px] font-mono text-[#c8ff00] pt-1">TRAFFIC → OPTIMIZATION → DISPATCH → INCIDENT → RE-OPTIMIZATION → UPDATED ROUTE</div>
                    </div>
            ...
            {currentStep === 8 && (
              <div className="space-y-4 font-mono">
                <div className="border-b border-slate-800 pb-2">
                  <span className="text-xs text-[#c8ff00] font-bold uppercase">STEP 08 OF 08</span>
                ...
                <div className="p-3 bg-[#111111] border border-slate-800 space-y-2 text-xs">
                  <div className="text-[#c8ff00] font-bold uppercase">BASELINE VS Q-FLOW METRICS:</div>
                  <table className="w-full text-left text-[11px]">
                    <thead className="text-slate-400 border-b border-slate-800">
                      <tr>
                        <th className="p-1">METRIC</th>
                        <th className="p-1">BASELINE</th>
                        <th className="p-1 text-[#c8ff00]">Q-FLOW</th>
          ...
          <div className="pt-4 border-t border-slate-800 space-y-2 font-mono">
            <div className="flex items-center justify-between gap-2">
              <Button
                ...
                variant="outline"
                className="flex-1 border-slate-800 bg-[#111111] hover:border-[#c8ff00] hover:text-[#c8ff00] text-slate-200 text-xs rounded-none"
              >
              ...
              <Button
                ...
                className="flex-1 bg-[#c8ff00] hover:bg-[#b5e600] text-black font-bold text-xs rounded-none"
              >
            ...
            <div className="flex items-center justify-between gap-2">
              <Button
                onClick={restartDemo}
                variant="outline"
                className="flex-1 border-slate-800 bg-[#111111] hover:border-[#c8ff00] hover:text-[#c8ff00] text-slate-400 text-[11px] rounded-none"
              >
              ...
              <Button
                ...
                className={`flex-1 text-[11px] font-bold rounded-none ${
                  isAutoDemo
                    ? "bg-[#ff4d2d]/20 border-[#ff4d2d] text-[#ff4d2d]"
                    : "bg-[#c8ff00]/10 border border-[#c8ff00]/40 text-[#c8ff00] hover:bg-[#c8ff00]/20"
                }`}
              >
        ...
        <div className="lg:col-span-8 h-[600px] bg-[#07090c] border border-slate-800 relative">
          <OSMVectorNetworkMap
            ...
            incident={currentStep >= 5 && currentStep < 7 ? incident : null}
