...
import { JuryMapToolbar } from "./JuryMapToolbar";
import { validateRouteForRendering } from "@/utils/routeValidator";
import { ZoomIn, ZoomOut, Maximize2, Activity, AlertCircle, Compass, ShieldAlert } from "lucide-react";
...
export function OSMVectorNetworkMap({
  ...
  const cleanupTrafficRef = useRef<(() => void) | null>(null);

  // ── State ──
  const [isMapLoaded, setIsMapLoaded] = useState(false);
  const [basemapUnavailable, setBasemapUnavailable] = useState(false);
  const [trafficHealth, setTrafficHealth] = useState<TrafficHealthStatus>(INITIAL_TRAFFIC_HEALTH);
  const [activeVisibility, setActiveVisibility] = useState<LayerVisibilityState>({
    ...DEFAULT_LAYER_VISIBILITY,
    ...(layerVisibility || {}),
  });
  ...
  const handleAllOn = () => {
    const next: LayerVisibilityState = {
      basemap: true,
      roadNetwork: true,
      nodes: true,
      traffic: true,
  ...
  const handleResetView = () => {
    setActiveVisibility(DEFAULT_LAYER_VISIBILITY);
    onLayerVisibilityChange?.(DEFAULT_LAYER_VISIBILITY);
    handleFitNetwork();
  };

  // Route Diagnostics inspector data
  const routeDiagnostics = React.useMemo(() => {
    if (vrpRoutes && vrpRoutes.length > 0) {
      const primary = vrpRoutes[0];
      const val = validateRouteForRendering(primary);
      return {
        routeSource: currentStep === 4 ? "Baseline Solver" : (currentStep ?? 0) >= 6 ? "QPSO Optimizer" : "OSM Graph Routing",
        nodeCount: primary.node_ids?.length || primary.stops?.length || val.nodeCount,
        edgeCount: primary.edge_ids?.length || val.edgeCount,
        geometryValid: val.isValid,
        restrictionsPass: primary.validation?.constraints_valid ?? true,
        blockedAvoided: primary.validation?.blocked_edges_avoided ?? true,
        oneWayLegal: primary.validation?.one_way_legal ?? true,
        fallbackUsed: val.isFallback || (primary.validation?.used_fallback_geometry ?? false),
        trafficCostStatus: "DYNAMIC (Synchronized)",
      };
    }
    if (routeResponse) {
      const val = validateRouteForRendering(routeResponse);
      return {
        routeSource: "Dijkstra Engine",
        nodeCount: routeResponse.node_ids?.length || val.nodeCount,
        edgeCount: routeResponse.edge_ids?.length || val.edgeCount,
        geometryValid: val.isValid,
        restrictionsPass: true,
        blockedAvoided: true,
        oneWayLegal: true,
        fallbackUsed: false,
        trafficCostStatus: "LIVE_GRAPH",
      };
    }
    return null;
  }, [vrpRoutes, routeResponse, currentStep]);
  ...
  const renderCurrentRoute = useCallback(() => {
    ...
    // Validate route before drawing to map!
    const val = validateRouteForRendering(rr);
    if (!val.isValid) {
      console.error(`[Q-FLOW Map] A->B Route rejected: ${val.reason}`);
      setRouteValidationError(`ROUTE INVALID — NOT RENDERED: ${val.reason}`);
  ...
  useEffect(() => {
    ...
    // Explicitly configure the web worker URL to use the static worker in /public
    // to prevent Vite from returning the SPA index.html fallback for module workers.
    ...
    map.on("load", () => {
      ...
      // ── Mapbox Live Traffic Vector Tiles Layer & Real-Time Monitoring ──
      const mapboxToken = import.meta.env.VITE_MAPBOX_ACCESS_TOKEN;
      if (mapboxToken) {
        addTrafficSource(map, mapboxToken);
        addTrafficLayer(map, "nodes-layer");
      }
      cleanupTrafficRef.current = setupTrafficMonitoring(map, mapboxToken, (newHealth) => {
        setTrafficHealth(newHealth);
      });
  ...
  useEffect(() => {
    ...
    console.log(`[Q-FLOW Map] setData: ${edgeFeatures.length} edge features, ${nodeFeatures.length} node features. Bounds: [${minLng}, ${minLat}] -> [${maxLng}, ${maxLat}]`);
    if (minLng < maxLng && minLat < maxLat) {
  ...
  useEffect(() => {
    ...
    const isBaselineStep = currentStep === 4;
    const vrpVisible = isBaselineStep ? activeVisibility.baselineRoutes : activeVisibility.optimizedRoutes;
    const vrpVis = vrpVisible ? "visible" : "none";

    vrpRoutes.forEach((vr, idx) => {
      // Validate route before drawing to map — reject straight lines and invalid paths
      const val = validateRouteForRendering(vr);
      if (!val.isValid) {
        console.error(`[Q-FLOW Map] Route rejected for vehicle ${vr.vehicle_id}: ${val.reason}`);
        hasInvalid = true;
        return;
      }
      ...
      if (!map.getLayer(casingId)) {
        map.addLayer({
          ...
          type: "line",
          source: srcId,
          layout: { visibility: vrpVis },
          paint: { "line-color": "#060708", "line-width": 10.0, "line-opacity": 0.9 },
        });
      } else {
        map.setLayoutProperty(casingId, "visibility", vrpVis);
      }
      ...
      if (!map.getLayer(lineId)) {
        map.addLayer({
          ...
          source: srcId,
          layout: { visibility: vrpVis },
          paint: { "line-color": vr.color || "#00f0ff", "line-width": 7.0, "line-opacity": 1.0 },
        });
      } else {
        map.setLayoutProperty(lineId, "visibility", vrpVis);
      }
    ...
    if (hasInvalid) {
      setRouteValidationError("ROUTE INVALID — NOT RENDERED (Straight-line or unroutable geometry rejected)");
    } else {
      setRouteValidationError(null);
    }
  ...
  }, [vrpRoutes, isMapLoaded, currentStep, activeVisibility.baselineRoutes, activeVisibility.optimizedRoutes]);
  ...
  useEffect(() => {
    ...
    if (currentStep && currentStep >= 7 && rerouteResult && rerouteResult.original_route?.geometry) {
  ...
  useEffect(() => {
    ...
    if (map.getLayer("osm-basemap-layer")) {
      map.setLayoutProperty("osm-basemap-layer", "visibility", activeVisibility.basemap ? "visible" : "none");
    }

    // 2. Road Network Edges
    const netVis = activeVisibility.roadNetwork ? "visible" : "none";
    ["edges-casing", "edges-glow", "edges-debug", "edges-primary", "edges-secondary", "edges-local", "edges-click-target"].forEach((id) => {
      if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", netVis);
    });

    // 3. Graph Nodes
    if (map.getLayer("nodes-layer")) {
      map.setLayoutProperty("nodes-layer", "visibility", activeVisibility.nodes ? "visible" : "none");
    }

    // 4. Mapbox Traffic
    setTrafficVisibility(map, !!activeVisibility.traffic);

    // 5. Point-to-Point A->B Route
    const routeVis = activeVisibility.route ? "visible" : "none";
    ...
    // 6. Baseline & Optimized VRP Routes
    const isBaselineStep = currentStep === 4;
    const vrpVisible = isBaselineStep ? activeVisibility.baselineRoutes : activeVisibility.optimizedRoutes;
    const vrpVis = vrpVisible ? "visible" : "none";
    if (vrpRoutes) {
      vrpRoutes.forEach((_, idx) => {
        const casingId = `qflow-vrp-casing-${idx}`;
        const lineId = `qflow-vrp-line-${idx}`;
        if (map.getLayer(casingId)) map.setLayoutProperty(casingId, "visibility", vrpVis);
        if (map.getLayer(lineId)) map.setLayoutProperty(lineId, "visibility", vrpVis);
      });
    }

    // 7. Old Incident Route
    if (map.getLayer("qflow-old-route-layer")) {
      map.setLayoutProperty("qflow-old-route-layer", "visibility", activeVisibility.oldRoute ? "visible" : "none");
    }

    // 8. Vehicle Markers
    const vehVis = activeVisibility.vehicles ? "" : "none";
    vehicleMarkersRef.current.forEach((m) => {
      m.getElement().style.display = vehVis;
    });

    // 9. Central Depot Marker
    if (depotMarkerRef.current) {
      depotMarkerRef.current.getElement().style.display = activeVisibility.depot ? "" : "none";
    }

    // 10. Delivery Point Markers
    const stopVis = activeVisibility.deliveryPoints ? "" : "none";
    deliveryMarkersRef.current.forEach((m) => {
      m.getElement().style.display = stopVis;
    });

    // 11. Incident Markers
    if (incidentMarkerRef.current) {
      incidentMarkerRef.current.getElement().style.display = activeVisibility.incidents ? "" : "none";
    }

    // 12. Turn Markers
    const turnVis = activeVisibility.turnMarkers ? "" : "none";
    turnMarkersRef.current.forEach((m) => {
      m.getElement().style.display = turnVis;
    });

    // 13. Map Labels
    const labelVis = activeVisibility.labels ? "visible" : "none";
    ["nodes-label", "road-labels", "osm-labels"].forEach((id) => {
      if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", labelVis);
    });
  }, [activeVisibility, isMapLoaded, vrpRoutes, currentStep]);
  ...
  return (
    <div className="relative w-full h-full min-h-[520px] bg-[#07090c] border border-slate-800 rounded-none overflow-hidden select-none font-sans">
      ...
      {basemapUnavailable && activeVisibility.basemap && (
      ...
      {/* Route Rejection Warning Banner */}
      {routeValidationError && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 px-4 py-2 bg-rose-950/95 border-2 border-rose-500 text-rose-200 text-xs font-mono font-bold rounded shadow-2xl flex items-center gap-2">
          <AlertCircle className="w-4 h-4 text-rose-400 shrink-0" />
          <span>{routeValidationError}</span>
        </div>
      )}

      {/* Bottom Jury Demo Map Layer Toolbar */}
      {(showJuryToolbar ?? (currentStep !== undefined)) && (
        <JuryMapToolbar
